...
var currentQuote = "";
function newQuote() {
  //get a random number btwn 0 & length of array to use as index
  var index = Math.floor((Math.random() * (rawQuotes.length + 1)));
  //if our index is 0 or even, then its a quote and we need to grab that + the author - the index after.
    if (index % 2 === 0 || index === 0) {
      currentQuote = rawQuotes[index] + rawQuotes[index+1];
      $("#quote").html(rawQuotes[index]);
      $("#author").html(rawQuotes[index+1]);
    } else {
    //if indx is odd, then it will pull an author so grab the quote right before in array
      currentQuote = rawQuotes[index-1] + rawQuotes[index];
      $("#quote").html(rawQuotes[index-1]);
      $("#author").html(rawQuotes[index]);
  }
  //create/refresh tweet button to include current quote
  $("#tweet-button").empty();
...
//load quote on page load and then pick new one each time button is clicked
newQuote();
